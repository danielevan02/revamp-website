  ...
  return (
    <motion.div
      className={cn(
        ...
        className,
        index === 1 ? "col-span-2" : 'col-span-2 md:col-span-1'
      )}
      style={{
        opacity: 0,
        y: 100,
    ...
    >
      {index === 1 ? (
        ...
        <Image src={header!} width={1000} height={1000} alt="logo" className="w-full max-h-40 object-cover rounded-lg" />
      )}
      <div
      ...
      >
        {icon}
        <div className={cn("font-sans font-bold text-neutral-600 dark:text-neutral-200 mb-2 mt-2", index === 1 && 'max-w-44')}>{title}</div>
        <div className={cn("font-sans font-normal text-neutral-600 text-xs dark:text-neutral-300", index === 1 && 'max-w-44')}>{description}</div>
      </div>
    </motion.div>
  );
};
